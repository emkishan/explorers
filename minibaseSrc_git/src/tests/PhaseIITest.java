		    ...
		    try 
		     {
		       ...
		       int numOfColms = parser.getNumberOfColms();
...
//						/System.out.println("projlistIndex="+projlistIndex); 
		...
		System.out.println("Enter total memory");
		int memory = (int)scanner.nextLong();
		...
		for(int k=0;k<numOfTables;k++){
			...
			try {
				System.out.println(projlistIndex);
			  	iscan = new IndexScan(new IndexType(IndexType.B_Index), indexNameList[k]+".in", indexNameList[k], attrTypeList[k], stringSizesList[k], numOfColsList[k], projlistIndex, 
			  			projList, condExprList, joinedColList[k], true);
		...
		IndexType[] b_index = new IndexType[indexNameList.length];
		for(int i = 0;i<b_index.length;i++){
			b_index[i] = new IndexType (IndexType.B_Index);
		}
