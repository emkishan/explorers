...
class JoinsDriver2 implements GlobalConst {
	...
	public JoinsDriver2() {
		...
		sailors.addElement(new Sailor(51, "Bob Holloway", 9, 53.6));
		...
		for (int i = 0; i < numsailors; i++) {
			try {
				...
				if (tempScore >= 1.0f)
					tempScore = 0.1f;
				System.out.println("Tuple : " + t.getStrFld(2) + "\t"
						+ t.getIntFld(1) + "\t" + t.getScore());
			} catch (Exception e) {
				System.err
		...
		for (int i = 0; i < numboats; i++) {
			try {
				...
				if (tempScore <= 0.0f)
					tempScore = 1.0f;
				System.out.println("Tuple : " + t.getIntFld(1) + "\t"
						+ t.getScore());
			} catch (Exception e) {
	...
	public void Query2() {
		...
		try {
			nlj = new TopNestedLoopsJoins(Stypes2, 2, Ssizes, Rtypes, 3,
					Rsizes, 10, am, "reserves.in", outFilter, null, proj1, 1, 5);
		} catch (Exception e) {
		...
		try {
			nlj2 = new TopNestedLoopsJoins(Jtypes, 2, Jsizes, Btypes, 3,
					Bsizes, 10, nlj, "boats.in", outFilter2, null, proj2, 1, 5);
		} catch (Exception e) {
			System.err.println("*** Error preparing for nested_loop_join");
			System.err.println("" + e);
			Runtime.getRuntime().exit(1);
		...
		try {
			sort_names = new Sort(JJtype, (short) 1, JJsize,(iterator.Iterator) nlj, 1, ascending, JJsize[0], 5);
		} catch (Exception e) {
			System.err.println("*** Error preparing for nested_loop_join");
			System.err.println("" + e);
			Runtime.getRuntime().exit(1);
