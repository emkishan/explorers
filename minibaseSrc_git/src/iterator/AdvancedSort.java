 ...
 * to clean up.
 */
public class AdvancedSort extends Iterator implements GlobalConst
{
  ...
  public void createTupleWithRID(Tuple toTuple, Tuple fromTuple, RID rid){
	  try{
	  int ridNum = rid.pageNo.pid*1000 + rid.slotNo;
	  AttrType tempIn[] = new AttrType[_in.length];
	  int i;
	  for(i=0;i<_in.length-1;i++)
		  tempIn[i] = _in[i];
	  tempIn[i] = new AttrType(AttrType.attrInteger);
	  toTuple.setHdr((short)(n_cols), tempIn, str_lens);
	  for(i=0;i<n_cols;i++){
		  switch(tempIn[i].attrType){
		  case AttrType.attrInteger:
			  if(i+1==n_cols)
				  toTuple.setIntFld(i+1, ridNum);
			  else
	  ...
	  toTuple.setScore(fromTuple.getScore());
	 // System.out.println("Tuple after adding RID");
	  //toTuple.print(tempIn);
	  }
	  catch(Exception e){
		  System.out.println("Exception in createTupleWithRID");
		  e.printStackTrace();
	  }
  }
	   ...
	   Exception
  {
    ...
    Tuple lastElem = new Tuple(tuple_size+4);  // need tuple.java
    AttrType tempIn[] = new AttrType[_in.length+1];
    for(int i=0;i<_in.length;i++){
    	tempIn[i] = _in[i];
    }
    tempIn[_in.length] = new AttrType(AttrType.attrInteger);
    try {
      lastElem.setHdr((short)(n_cols+1), tempIn, str_lens);
    }
  ...
  //===========================
    ...
    while ((p_elems_curr_Q + p_elems_other_Q) < max_elems) {
      try {
    	  Tuple tempTuple = _am.get_next(rid);// according to Iterator.java
    	  if(tempTuple==null)
    		  tuple=null;
    	  else{
    		  tuple = new Tuple();
    		  createTupleWithRID(tuple,tempTuple,rid);
    	  }  
	//System.out.println("First RID Page: " + rid.pageNo + " Slot No: " + rid.slotNo);
	} catch (Exception e) {
	e.printStackTrace(); 
   ...
   * @exception SortException something went wrong in the lower layer. 
   */
  public AdvancedSort(AttrType[] in,         
	      short      len_in,             
	      short[]    str_sizes,
	      FileScan   am,                 
	      int        sort_fld,          
  ...
  {
    _in = new AttrType[len_in+1];
    n_cols = (short)(len_in+1);
    int n_strs = 0;
    ...
    _in[len_in] = new AttrType(AttrType.attrInteger);
    ...
    try {
      t.setHdr((short)(len_in+1), _in, str_sizes);
    }
    catch (Exception e) {
