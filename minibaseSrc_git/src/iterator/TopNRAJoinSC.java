...
public class TopNRAJoinSC {
	...
	public TopNRAJoinSC(int numTables, AttrType[][] in, int[] len_in,
...
//		getTopK();
	...
	public void AddTopNRAJoin1(ArrayList itrList,ArrayList relationList) throws IOException{
	
		for(int i=0;i<itrList.size();i++){
		Iterator itr = (Iterator)itrList.get(i);
		int relation = (int)relationList.get(i);
		Tuple t1;
		try {
			while((t1 =itr.get_next())!=null){
				t1.print(inputRelations[relation]);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	    ...
	    try {
			...
			do{
				for (int i = 0; i < numberOfTables; i++) {
					...
					System.out.println("RID is : "+iterators[relNum]);*/
					
					/*System.out.println("@@@@@@@@@@  at start");
					tuple.print(inputRelations[relNum]);*/
					scorePerRelation[relNum] = tuple.getFloFld(inputRelations[relNum].length-1); //gets the score from the tuple
					...
					case AttrType.attrString:
					    ...
					    while ( dupTemp != null) {
						    ...
						    dupScan.closescan();
					    //create btree file, then a scan for it, open scan read join column, put this as key in index 
					    String key = tuple.getStrFld(joinColumns[relNum]+1);
						interTuple.setStrFld(joinColumns[0]+1, key);
						boolean too = true;
						Tuple dup2 = new Tuple();
						dup2.setHdr((short)(numberOfTables+1), dupInterAttr, strSizes);
						...
						HashMap<String, Boolean> randomMap = new HashMap<String, Boolean>();
							...
							if ((dup2) != null) {
								...
								// updateTuple(tuple, dup2, relNum, tupleOffset,1);
								// dup2.setIntFld((relNum+1),key);
								
								
								
								int isSet = dup2.getIntFld((relNum + 2));
								...
								if (isSet == 1) {
									...
									if((t = interScan.get_next())!=null){
										...
										Tuple newTuple = new Tuple();
										newTuple.setHdr((short)attrCount, interAttr, strSizes);
										//updateTuple(t,newTuple, relNum, tupleOffset,1);
										newTuple.tupleCopy(t);
										/*System.out.println("$$$$After$$$$$$$$");
										newTuple.print(interAttr);*/
										updateTuple(tuple, newTuple, relNum, tupleOffset,1);
									...
									//thus increment by relNum+2
									dup2.setIntFld((relNum + 2), 1);
									dupCheckIndexFile.updateRecord(dupTempRID, dup2);
									//dup2.print(dupInterAttr);
									while((t = interScan.get_next())!=null){
										tempRID = interScan.getRID();
										...
										t.print(interAttr);*/
										//tuple.print(inputRelations[relNum]);
										updateTuple(tuple, t, relNum, tupleOffset,1);
										bestScore=0.0f;
										worstScore=0.0f;
										for(int j=0;j<numberOfTables;j++){
											if(t.getStrFld(getTupleOffset(j)+joinColumns[j]+1)!=null){
							...
							//thus increment by relNum+2
							dupFileTemp.setStrFld(1,key);
							dupFileTemp.setIntFld((relNum+2),1);
							/*System.out.println("prtinting before inserting ");
							dupFileTemp.print(dupInterAttr);*/
							dupCheckIndexFile.insertRecord(dupFileTemp.getTupleByteArray());
							//tuple.print(inputRelations[relNum]);
							worstScore = tuple.getFloFld(inputRelations[relNum].length-1); //gets the score from the tuple
							...
							//newTuple.setStrFld(getTupleOffset(relNum)+joinColumns[relNum]+1, key);
							newTuple.setFloFld(attrCount-1, worstScore);
							newTuple.setFloFld(attrCount-2, bestScore);
							newTuple.setStrFld(attrCount, key);
	...
	public int calculateIndicator(){
		...
		FldSpec[] fldProjectionList = new FldSpec[attrCount];
		int tableIndex =-1;
		...
		try {
			...
			IndexScan first =	new IndexScan(new IndexType(IndexType.B_Index), "InterTuple.in", "BTreeIndex", interAttr, strSizes, attrCount, attrCount, tProjection, null, attrCount, false);
								//new IndexScan(new IndexType(IndexType.B_Index), "TempResults.in","BTreeIndex", combinedAttr, combinedStrSizes, combinedAttrCount,combinedAttrCount, combinedProjection, null, combinedAttrCount, false);
			
			IndexScan second = new IndexScan(new IndexType(IndexType.B_Index), "InterTuple.in", "BTreeIndex", interAttr, strSizes, attrCount, attrCount, tProjection, null, attrCount, false);
							   //new IndexScan(new IndexType(IndexType.B_Index), "TempResults.in","BTreeIndex", combinedAttr, combinedStrSizes, combinedAttrCount,combinedAttrCount, combinedProjection, null, combinedAttrCount, false);
			   
			int counter = this.topK;
			...
			second.close();
			first.close();		
			FileScan fileScan =new FileScan("InterTuple.in", interAttr, strSizes,(short)attrCount, attrCount,tProjection, null);
			...
			while ((tempTuple = fileScan.get_next()) != null) {
				...
				for (int j = 0; j < numberOfTables; j++) {
					...
					if (flag !=0) {
						...
						currScore[j] = tempStreamTuple.getFloFld(getTupleOffset(j)+inputRelations[j].length-1);
					}
				}
			}
			fileScan.close();
	...
	public void calculateIndicator1()
	{
		...
		try {
			...
			FileScan fScan = null;
			try {
				fScan = new FileScan("InterTuple.in", interAttr, strSizes,(short)attrCount, attrCount,tProjection, null);
			...
			try {
				topIterator = new Sort(interAttr, (short)attrCount, strSizes,fScan, attrCount-1,order, 4, n_buf_pgs);
				//new Sort(combinedAttr,(short) combinedAttrCount, combinedStrSizes, fScan,combinedAttrCount-1, order, 4, 20);				  
			...
			fScan.close();
			FileScan fileScan =
					new FileScan("InterTuple.in", interAttr, strSizes,(short)attrCount, attrCount,tProjection, null);
					//new FileScan("TempResults.in",combinedAttr, combinedStrSizes, (short) combinedAttrCount,combinedAttrCount, fldProjectionList, null);
